...
let VanillaCalendar = (function () {
    function VanillaCalendar(options) {
            ...
            if (opts.date.getTime() <= opts.todaysDate.getTime() - 1 && !opts.pastDates) {
                ...
                if (opts.datesFilter) {
                    if (available_week_day.length) {
                        ...
                        newDayElem.classList.add('vanilla-calendar-date--disabled')
                        newDayElem.classList.add('bg-warning')



                        let approveWord = document.createElement('h6')

                        approveWord.classList.add('approveWord')

                        var iddate  = (date.getFullYear() + '-' + String(date.getMonth() + 1).padStart('2', 0) + '-' + String(date.getDate()).padStart('2', 0))
                        approveWord.setAttribute('id' , iddate + '-1')
                        approveWord.appendChild(document.createTextNode('Approved'));


                        let unapproveWord = document.createElement('h6')

                        unapproveWord.classList.add('unapproveWord')
                        unapproveWord.setAttribute('id' , iddate + '-2')
                        unapproveWord.appendChild(document.createTextNode('UnApproved'));

                        dateElem.append(approveWord)
                        dateElem.append(unapproveWord)



                        // newDayElem.classList.add('disable-date')
                        }
                        else {
        ...
        const monthNext = function () {
            ...
            var approveWord = document.getElementById("2019-10-17-1");

            var unapproveWord =document.getElementById("2019-10-17-2");
    
            if(approveWord != null) {
                unapproveWord.style.display = "none";
                approveWord.style.display = "block";
                }
